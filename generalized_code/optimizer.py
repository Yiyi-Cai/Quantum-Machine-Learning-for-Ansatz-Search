...
class CircuitOptimizer:
    ...
    def __init__(self, pop_size, target, one_q_gates, two_q_gates, max_moments, n_qubits, dimension):
        self.population = Population()
        self.population.fill_population(pop_size, target, one_q_gates, two_q_gates, max_moments, n_qubits, dimension)
        self.generation = 0
        self.pop_size = pop_size
        self.one_q_gates = one_q_gates
        self.two_q_gates = two_q_gates
        self.parents = []
        parents_size = int(np.ceil(pop_size * 0.5))
        if parents_size < 2:
            self.parents_size = 2
        else:
    ...
    def print_info(self):
        self.population.update()
        print("Generation: {} Fittest: {} Fitness before Log {}".format(self.generation, self.population.fittest.get_fitness(), 1-np.exp(-self.population.fittest.get_fitness())))
    ...
    def crossover_pair(self, parent1, parent2):
        ...
        params1 = (2.*np.pi)*np.random.random(get_num_params(tensor1, self.n_qubits, self.one_q_gates, self.two_q_gates))
        params2 = (2.*np.pi)*np.random.random(get_num_params(tensor2, self.n_qubits, self.one_q_gates, self.two_q_gates))
        
        self.offsprings.add_circuit(IndividualCircuit(tensor1, self.target, self.one_q_gates, self.two_q_gates, self.n_qubits, self.dimension))
        self.offsprings.add_circuit(IndividualCircuit(tensor2, self.target, self.one_q_gates, self.two_q_gates, self.n_qubits, self.dimension))
    ...
    # identical_circuits = 0
        for i in range(len(self.offsprings.individuals)):
            ...
            mutate_gate = random.randint(0, self.one_q_gates-1)
            ...
            if random.randint(0, 1):
                ...
                for q in range(len(curr_moment)):
                    ...
                    if not any(curr_qubit[self.one_q_gates:]):
                ...
                for i in range(self.one_q_gates + self.two_q_gates):
            ...
            # mutate two qubit gate
                ...
                two_gates_idx = self.one_q_gates + random.randrange(0, self.two_q_gates*2, 2)
            ...
            for j in range(len(self.population.individuals)):
                # same = self.population.individuals[j].tensor == curr_offspring.tensor
                try: 
                    same = self.population.individuals[j].tensor == curr_offspring.tensor
                except:
                    same = (np.array(self.population.individuals[j].tensor) == np.array(curr_offspring.tensor)).all()
                if same:
