...
class IndividualCircuit:
    ...
    def __init__(self, tensor, target, num_one_gates, num_two_gates, n_qubits):
        self.tensor = tensor
    ...
    def draw(self, title=""):
        ...
        qnode = qml.QNode(convert_tensor_to_circuit, dev)
        fig, ax = qml.draw_mpl(qnode)(self.tensor, self.params, self.n_qubits, self.num_one_gates, self.num_two_gates, self.dev_wires)
        ax.set_title(title)
        fig.show()
    ...
    def loss_1d(self, circuit, params):
        return self.jensen_shannon_loss(circuit(self.tensor, params, self.n_qubits, self.num_one_gates, self.num_two_gates, self.dev_wires), self.target)
    
    def loss_2d(self, circuit, params):
        return self.jensen_shannon_loss(circuit(self.tensor, params, self.n_qubits, self.num_one_gates, self.num_two_gates, self.dev_wires).reshape(16,16),self.target.reshape(16,16))
    
    def loss_3d(self, circuit, params):
        return self.jensen_shannon_loss(circuit(self.tensor, params, self.n_qubits, self.num_one_gates, self.num_two_gates, self.dev_wires).reshape(16,16,16),self.target.reshape(16,16,16))
